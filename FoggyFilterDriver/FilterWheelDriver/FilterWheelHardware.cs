...
//
// ASCOM FilterWheel hardware class for FoggyAstroFilterWheel
//
...
namespace ASCOM.FoggyAstroFilterWheel.FilterWheel
{
    ...
    internal static class FilterWheelHardware
    {
        ...
        internal static Serial serialPort = new Serial();
        ...
        static FilterWheelHardware()
        {
            try
            {
                ...
                tl = new TraceLogger("", "FoggyAstroFilterWheel.Hardware");
        ...
        public static string Name
        {
            ...
            get
            {
                string name = "FoggyAstroFWDriver";
        ...
        private static int[] fwOffsets = new int[5] { 0, 0, 0, 0, 0 }; //class level variable to hold focus offsets
        private static string[] fwNames = new string[5] { "Red", "Green", "Blue", "Clear", "Other" }; //class level variable to hold the filter names
        ...
        internal static short Position
        {
            get
            {
                //Get filter number from the wheel, we can't rely on fwPosition being up
                //to date as the buttons on the wheel may have been pressed
                String f = SerialCmd("GETFILTER#");
                f = f.Substring(f.Length - 1);
                fwPosition = short.Parse(f);
            ...
            set
            {
                ...
                String f = SerialCmd("FILTER"+value+"#");
                f = f.Substring(f.Length - 1);
                fwPosition = short.Parse(f);
            }
        }

        internal static String SerialCmd(String cmd)
        {
            lock (_serialPortLock)
            {
                String response;
                //CheckConnected("Attempting to communicate on com port");
                try
                {
                    serialPort.ClearBuffers();
                    tl.LogMessage("Sending serial command: ",cmd);
                    serialPort.Transmit(cmd);

                    response = serialPort.ReceiveTerminated("#");
                    tl.LogMessage("Received serial reponse",response);
                ...
                {
                    tl.LogMessage("Error communicating on serial port: ",ex.ToString());
        ...
        internal static void ReadProfile()
        {
            using (Profile driverProfile = new Profile())
            {
                ...
                for(int i = 0; i < fwNames.Length; i++)
                {
                    fwNames[i] = driverProfile.GetValue(DriverProgId, "f" + i.ToString() + "_name", string.Empty, fwNames[i]);
                }
            }
        }
        ...
        internal static void WriteProfile()
        {
            using (Profile driverProfile = new Profile())
            {
                ...
                for (int i = 0; i < fwNames.Length; i++)
                {
                    driverProfile.WriteValue(DriverProgId, "f" + i.ToString() + "_name", fwNames[i]);
                }
            }
        }
